...
export function parseListing(
  ...
  const galleryImages = $("#altImages li:not(.videoThumbnail):not(.multiple-videos) img")
    .map((_, el) => $(el).attr("src") || "")
    .get()
    .filter(Boolean);

  const hasVideo =
    $("#altImages li.videoThumbnail, #altImages li.multiple-videos").length > 0;

  const aplusModuleCount = $("#aplus, #aplus_feature_div .aplus-module").length;
  const isPlaceholderImg = (url: string) =>
    /grey-pixel\.gif|transparent-pixel|spacer\.gif/i.test(url);
  const aplusImageUrls = $("#aplus img, #aplus3p_feature_div img")
    // Amazon lazy-loads A+ images: `src` holds a grey-pixel placeholder
    // until JS swaps in `data-src`, so data-src must win when present.
    .map((_, el) => $(el).attr("data-src") || $(el).attr("src") || "")
    .get()
    .filter((url) => Boolean(url) && !isPlaceholderImg(url));
